...
void
macb_start_transmission_queue(struct macb_queue *queue)
{
	prism_sp_tx_queue_writel(queue, SP_TER, 1 << PRISM_SP_TX_TXR_START_BITN);
}
...
static void macb_init_buffers(struct macb *bp)
{
	...
	for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
		prism_sp_desc_set_rbqp(queue, queue->rx_ring_dma);
	}
	for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
		prism_sp_desc_set_tbqp(queue, queue->tx_ring_dma);
	}
	// For the Prism SP, the code below is not necessary.
	for (q = 0, queue = bp->queues; q < bp->num_hw_queues; ++q, ++queue) {
			     ...
			     bool tx_pause, bool rx_pause)
{
	...
	if (!(bp->caps & MACB_CAPS_MACB_IS_EMAC)) {
		...
		for (q = 0, queue = bp->queues; q < bp->num_hw_queues; ++q, ++queue) {
			queue_writel(queue, IER, bp->rx_intr_mask | bp->tx_intr_mask | MACB_BIT(HRESP));
		}
		for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
			prism_sp_gem_queue_enable_irqs(queue);
		}
	}
...
static void macb_tx_error_task(struct work_struct *work)
{
	...
	printk("Prism: %s: Not implemented for external FIFO interface"
		" with Prism SP. Beware of unexpected behavior.\n", __func__);
	...
	queue_writel(queue, IER, bp->tx_intr_mask);
	...
	macb_start_transmission_queue(queue);
...
static void gem_rx_refill(struct macb_queue *queue)
{
	...
	// Announce to the SP that there are descriptors available again.
	prism_sp_rx_queue_writel(queue, SP_TER, 1 << PRISM_SP_RX_TXR_START_BITN);
...
int gem_rx(struct macb_queue *queue, struct napi_struct *napi,
		  int budget)
{
	struct macb *bp = queue->bp;
	unsigned int		len;
...
bool macb_rx_pending(struct macb_queue *queue)
{
	struct macb *bp = queue->bp;
	unsigned int		entry;
	struct macb_dma_desc	*desc;
...
static u32 macb_dbw(struct macb *bp)
{
	...
	// External FIFO mode always uses 32 bit
...
{
	static void *iou_slcr_regs = NULL;
	u32 config;
	...
	if (iou_slcr_regs == NULL) {
		iou_slcr_regs = ioremap(0xff180000, 0x1000);
		printk("Prism: %s: Mapped 0xff180000 to %px\n", __func__, iou_slcr_regs);
		*(volatile uint32_t *)((uint8_t *)iou_slcr_regs + 0x308) = 0x00042529;
	}
	gem_writel(bp, EFO, 0x1);
	bp->efo = true;
}
...
static int macb_open(struct net_device *dev)
{
	...
	for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
		if (queue->sp_rx != NULL) {
			if (queue->sp_rx->booted) {
				rproc_shutdown(queue->sp_rx->rproc);
			}
			rproc_boot(queue->sp_rx->rproc);
			queue->sp_rx->booted = 1;
		}
		else {
			printk("Prism: %s: No Prism SP RX available.\n", __func__);
		}

		if (queue->sp_tx != NULL) {
			if (queue->sp_tx->booted) {
				rproc_shutdown(queue->sp_tx->rproc);
			}
			rproc_boot(queue->sp_tx->rproc);
			queue->sp_tx->booted = 1;
		}
		else {
			printk("Prism: %s: No Prism SP TX available.\n", __func__);
		}
...
{
	// We hardcode this for the Prism SP.
	*queue_mask = 0x1;
	*num_queues = 1;
}
...
static int macb_init(struct platform_device *pdev)
{
	...
	for (hw_q = 0, q = 0; hw_q < MACB_MAX_QUEUES; ++hw_q) {
		...
		netif_napi_add(dev, &queue->napi_rx, macb_rx_poll_split_irq);
		netif_napi_add(dev, &queue->napi_tx, macb_tx_poll_split_irq);
		...
		err = prism_macb_init_queue(pdev, queue, q);
		if (err)
			return err;
...
static int macb_probe(struct platform_device *pdev)
{
	...
	macb_probe_queues(mem, native_io, &queue_mask, &num_hw_queues);
	// Check whether there are Prism SPs.
	if ((num_queues = prism_sp_count_queues(pdev)) != 0) {
		uses_prism_sp = true;
	}
	else {
		num_queues = num_hw_queues;
		uses_prism_sp = false;
	}
	...
	bp = netdev_priv(dev);
	bp->efo = false;
	bp->uses_prism_sp = uses_prism_sp;
	bp->pdev = pdev;
	bp->dev = dev;
	bp->regs = mem;
	...
	bp->num_hw_queues = num_hw_queues;
	...
	// Remove the receive-complete and transmit-complete interrupt bits
	// from the masks in case of split IRQs.
	bp->rx_intr_mask &= ~((uint32_t)MACB_BIT(RCOMP) | MACB_BIT(ISR_ROVR));
	bp->tx_intr_mask &= ~(uint32_t)MACB_BIT(TCOMP);
...
static int __maybe_unused macb_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(&bp->dev->dev)) {
		...
		macb_writel(bp, NCR, ctrl);
		/* Tie off RX queues */
		for (q = 0, queue = bp->queues; q < bp->num_hw_queues;
		     ++q, ++queue) {
			if (bp->caps & MACB_CAPS_QUEUE_DISABLE)
		...
		for (q = 1, queue = bp->queues; q < bp->num_hw_queues;
		     ++q, ++queue) {
			queue_writel(queue, IDR,
				bp->rx_intr_mask |
				bp->tx_intr_mask |
...
static struct platform_driver spgem_driver = {
	.probe		= macb_probe,
	.remove		= macb_remove,
	.driver		= {
		.name		= "spgem",
...
module_platform_driver(spgem_driver);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Prism SP/GEM Ethernet driver");
MODULE_AUTHOR("Haavard Skinnemoen (Atmel)");
MODULE_ALIAS("platform:spgem");
