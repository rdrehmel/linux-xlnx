...
#define GEM_EFO			0x004c /* External Fifo Interface */
...
#define GEM_AUTOFLUSHCNT	0x01b4 /* Receive DMA Flushed Packets */
...
struct macb_queue {
	...
	unsigned int SP_IDR;
	unsigned int SP_IMR;
	unsigned int SP_QP_LSB;
	unsigned int SP_QP_MSB;
...
#define prism_sp_rx_queue_readl(queue, reg)			readl((queue)->sp_rx->regs + (queue)->reg)
#define prism_sp_rx_queue_writel(queue, reg, value)	writel((value), (queue)->sp_rx->regs + (queue)->reg)
#define prism_sp_tx_queue_readl(queue, reg)			readl((queue)->sp_tx->regs + (queue)->reg)
#define prism_sp_tx_queue_writel(queue, reg, value)	writel((value), (queue)->sp_tx->regs + (queue)->reg)

struct macb {
	void __iomem		*regs;
	bool			native_io;

	bool efo; /* External FIFO interface active */
	bool uses_prism_sp;
